...
colnames(GDP_per_capita) = c("Year", "GDP_per_capita", "Country")
...
life_expectancy_GDP <- left_join(life_expectancy, GDP_per_capita, by = c("Country","Year"))
which(is.na(life_expectancy_GDP[,'GDP']))
...
set.seed(1234)
stepwise.fit<-train(Salary~AtBat+HmRun+Runs+RBI+Walks+poly(Years,5)+CAtBat+CHits+CHmRun+CRuns+CRBI+CWalks+
                    NewLeague+Division+PutOuts+Assists+Errors,
                  data=training,
...
stepwise.validate<-postResample(pred=stepwise.pred,obs=validate$Salary)
stepwise.validate

plot(stepwise.pred,validate$Salary)
lines(0:2000,0:2000)
...
#
set.seed(1234)
tree.fit<-train(Salary~AtBat+HmRun+Runs+RBI+Walks+Years+CAtBat+CHits+CHmRun+CRuns+CRBI+CWalks+
                      NewLeague+Division+PutOuts+Assists+Errors,
                    data=training,
...
set.seed(1234)
knn.fit<-train(Salary~AtBat+HmRun+Runs+RBI+Walks+Years+CAtBat+CHits+CHmRun+CRuns+CRBI+CWalks+PutOuts+Assists+Errors,
                data=training,
...
#Computing Errror Metrics
knn.validate<-postResample(pred=knn.pred,obs=validate$Salary)
knn.validate

plot(knn.pred,validate$Salary)
lines(0:2000,0:2000)
