...
## Year - need to discuss if we should restrict to 2000 to 2014 (instead of 2015) because of missing data in 2015; maybe also remove 2013?
```{r Year as Factor}
...
Missing values: 200 N/A (out of almost 3,000 observations) - most of the missing data is from the most recent year (2015). One country, South Sudan, doesn't have any data for all the years  (it seems that alcohol was illegal during this timeframe - we could set these values to 0)
Data accuracy: The data seems to make sense (reasonable min and max, low consumption in mainly Muslim countries, for example)
...
Missing values: very small number of missing values (19 in the whole data set)
...
#investigate minimum population value
...
names(wb_pop)[1] <- "Country"

#Columns that need to be renamed in order to merge (WB data is missing Cook Islands and Niue)
wb_data[,'Country'] <- sub("Bahamas, The", "Bahamas", wb_data$Country) 
wb_data[,'Country'] <- sub("Bolivia", "Bolivia (Plurinational State of)", wb_data$Country)
wb_data[,'Country'] <- sub("Congo, Rep.", "Congo", wb_data$Country) 
wb_data[,'Country'] <- sub("Cote d'Ivoire", "Côte d'Ivoire", wb_data$Country) 
wb_data[,'Country'] <- sub("Czech Republic", "Czechia", wb_data$Country) 
wb_data[,'Country'] <- sub("Korea, Dem. People's Rep.", "Democratic People's Republic of Korea", wb_data$Country) 
wb_data[,'Country'] <- sub("Congo, Dem. Rep.", "Democratic Republic of the Congo", wb_data$Country) 
wb_data[,'Country'] <- sub("Egypt, Arab Rep.", "Egypt", wb_data$Country) 
wb_data[,'Country'] <- sub("Gambia, The", "Gambia", wb_data$Country) 
wb_data[,'Country'] <- sub("Iran, Islamic Rep.", "Iran (Islamic Republic of)", wb_data$Country) 
wb_data[,'Country'] <- sub("Kyrgyz Republic", "Kyrgyzstan", wb_data$Country) 
wb_data[,'Country'] <- sub("Lao PDR", "Lao People's Democratic Republic", wb_data$Country) 
wb_data[,'Country'] <- sub("Micronesia, Fed. Sts.", "Micronesia (Federated States of)", wb_data$Country) 
wb_data[,'Country'] <- sub("Korea, Rep.", "Republic of Korea", wb_data$Country) 
wb_data[,'Country'] <- sub("Moldova", "Republic of Moldova", wb_data$Country) 
wb_data[,'Country'] <- sub("St. Kitts and Nevis", "Saint Kitts and Nevis", wb_data$Country) 
wb_data[,'Country'] <- sub("St. Lucia", "Saint Lucia", wb_data$Country) 
wb_data[,'Country'] <- sub("St. Vincent and the Grenadines", "Saint Vincent and the Grenadines", wb_data$Country) 
wb_data[,'Country'] <- sub("Slovak Republic", "Slovakia", wb_data$Country) 
wb_data[,'Country'] <- sub("North Macedonia", "The former Yugoslav republic of Macedonia", wb_data$Country) 
wb_data[,'Country'] <- sub("United Kingdom", "United Kingdom of Great Britain and Northern Ireland", wb_data$Country) 
wb_data[,'Country'] <- sub("United States", "United States of America", wb_data$Country) 
wb_data[,'Country'] <- sub("Tanzania", "United Republic of Tanzania", wb_data$Country) 
wb_data[,'Country'] <- sub("Venezuela, RB", "Venezuela (Bolivarian Republic of)", wb_data$Country) 
wb_data[,'Country'] <- sub("Vietnam", "Viet nam", wb_data$Country) 
wb_data[,'Country'] <- sub("Yemen, Rep.", "Yemen", wb_data$Country) 

#Replacing null data with World Bank population data
#still working on this
#left_join(life_expectancy, Population, by = c("Country","Year")) 
```
...
Missing values: 10 countries are missing all the values, we may have to fill some of them manually
