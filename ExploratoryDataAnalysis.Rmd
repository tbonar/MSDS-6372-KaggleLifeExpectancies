...
life_expectancy_GDP <- left_join(life_expectancy, GDP_per_capita, by = c("Country","Year"), copy=TRUE)
# life_expectancy_GDP <- select(life_expectancy_GDP, everything(), GDP.x = GDP.y)
which(is.na(life_expectancy_GDP[,'GDP.x']))

# life_expectancy_GDP %>% ggplot(aes(x = Year, y = GDP)) + 
#   geom_point(size=.75) +
#   geom_jitter()
```
...
#Tidy Data
# wb_pop <- wb_population[4:267,]
# colnames(wb_pop)<- wb_population[3,]
# wb_pop <- wb_pop[c(1,45:60)]
# wb_pop <- gather(wb_pop, Year, Population, '2000':'2012' , factor_key=TRUE)
# wb_pop <- wb_pop %>% rename('Country' = 'Country Name')
# wb_pop$Year <- as.numeric(wb_pop$Year)

# Fill missing GDP data from secondary source: World Banks -- indicator code:  SP.POP.TOTL
wb_population <- wb_data(indicator = "SP.POP.TOTL", country = "countries_only", start_date = 2000, end_date = 2012)
# Grab only necessary columns and map to match dataset
wb_population <- wb_population %>% select(date, SP.POP.TOTL, country)
colnames(wb_population) = c("Year", "Population", "Country")
...
cleandata <- function(wb_dataset, column_name) {
  wb_column <<- paste(column_name, "y", sep=".")
  orig_column <<- paste(column_name, "x", sep=".")
  
  #merging datasets: Life Expectancy and World Bank
  joined <<- left_join(life_expectancy, wb_dataset, by = c("Country","Year")) 
  #Loop through joined dataset
  for (i in 1:length(joined)){
    if ((!is.na(joined[i,wb_column])) & ((joined[i,orig_column] != joined[i,wb_column]) | (is.na(joined[i,orig_column])))){
      life_expectancy[i,column_name] <<- joined[i,wb_column]
    }
  }
}

cleandata(wb_pop,"Population")
investigate_missing_column_values("Population")
miss_var_summary(life_expectancy)
```
